...
# Python Ver:   3.11.5
#
# Author:       Krystle Tsuda
#
# Purpose:      Phonebook Project. Demonstrating OOP, Tkinter GUI module,
...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit aplication?"):
        # this closes app
        self.master.destroy()
        os._exit(0)
...
def first_run(self):
    data = ('John','Doe','John Doe','111-111-1111','jdoe@emial.com')
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
...
def onSelect(self,event):
    ...
    with conn:
        ...
        # this returns a tuple and we can slive it into 4 parts using dat[] during the iteration
        for data in varBody:
...
def addToList(self):
    ...
    #normalize the dat to keep it consistent in the database
    var_fname = var_fname.strip() # this will remove any blank spaces bvefore and after the users entry
    var_lname = var_lname.strip()
    var_fname = var_fname.title() # this iwll ensure the the first charcter in each word is capitalized
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.curselection()) # Listboxs selected value
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()  
...
######              onRefresh(self) # update the listbox of the changes
            ...
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the database and canot be deleted at this time. \n\nPlease add another record first before you can delete ({}).".format(var_select,var_select))
    conn.close()
...
def onUpdate(self):
    ...
    # for name changes the suer will need to delete the entire record and start over.
    var_phone = self.txt_phone.get().strip() # normalize the data to maintain database integrity
    var_email = self.txt_email.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is dat present
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cur = conn.cursor()
        ...
        messagebox.showerror("Missing information","Please select a name from the list. \nThen edit the pohne or email information.")
    onClear(self)
